...
class I3D(nn.Module):
    ...
    def __init__(self, num_classes=16, dropout_drop_prob=0.5, in_channel=3, spatial_squeeze=True):
        super(I3D, self).__init__()
        self.features = nn.Sequential(
            BasicConv3d(in_channel, 64, kernel_size=7, stride=2, padding=3), # (64, 32, 112, 112)
...
if __name__ == '__main__':
    import numpy as np
    depth_seqs = torch.randn(1, 3, 8, 224, 224)
    model = I3D(num_classes=16, in_channel=1)
    # model = BasicConv3d(3, 64, kernel_size=7, stride=2, padding=3)
    out = model(depth_seqs)
    print(out.shape)
